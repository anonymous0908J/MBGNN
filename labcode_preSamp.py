...
class Recommender:
	def __init__(self, sess, handler):
		self.sess = sess
		# self.handler.trnMats, self.handler.tstInt, self.handler.trnLabel, self.handler.tstUsrs, args.behNum = datas
		self.handler = handler

		# # sparsity test
		# trnMat = np.sum(self.handler.trnMats)
		# trnnum = np.reshape(np.array(np.sum(trnMat, axis=1)), [-1])
		# chsn = (trnnum <= 2790) * (trnnum > 359)
		# newTstUsrs = list()
	...
	def ours(self):
		...
		# uEmbed0 = tf.nn.embedding_lookup
		ulats = [self.uEmbed0]
		ilats = [self.iEmbed0]
...
if __name__ == '__main__':
	...
	log('Start')
	handler = DataHandler()
	handler.LoadData(trans=True if args.preTrn_item else False)
	log('Load Data')
